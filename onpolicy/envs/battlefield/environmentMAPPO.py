...
class BattleField(AECEnv):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.agents = self.possible_agents_blue[:]
        self.rewards = self.rewards_blue
        self._cumulative_rewards = self._cumulative_rewards_blue 
        ...
        self.fixed_red = np.random.choice(np.arange(self.size ** 2, 2*self.size**2), self.num_agents)

        self.local_states_all = np.hstack((self.local_states_arr, self.fixed_red))
    ...
    def get_new_state(self, state, action, agent, oracle, size, mf):
        ...
        idx_prob_dist = oracle.get_transition_blue(state, action, mf['blue'], mf['red'])
...
# TODO: fix observations
    def step(self, jointAction):
        ...
        self.observations = {agent: np.hstack((mf['blue'], self.local_state[agent])) for agent in self.agents}
        reward = self.mf_oracle.ask_oracle_reward(mf['red'], mf['blue'])

        self.rewards_blue = {agent: reward for agent in self.agents_blue}
        self.rewards = self.rewards_blue
