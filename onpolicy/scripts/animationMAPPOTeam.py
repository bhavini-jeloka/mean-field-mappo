...
from onpolicy.envs.battlefield.environmentMAPPOTeam import BattleFieldEnv, BattleField
...
def main(args):
    ...
    if all_args.share_policy:
        from onpolicy.runner.separated.battlefield_runner import BattleFieldRunner as Runner
    else:
        raise NotImplementedError
    ...
    for team_id in range(num_teams):
        rnn_states[team_id] = np.zeros((1, * runner.buffer[team_id].rnn_states.shape[2:]), dtype=np.float32)
        masks[team_id] = np.ones((1, num_agents, 1), dtype=np.float32)
